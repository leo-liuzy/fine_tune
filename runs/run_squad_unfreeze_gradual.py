...
from modules import (WEIGHTS_NAME, BertTokenizer,
...
from modules.modeling_bert_unfreeze import BertConfig, BertForQuestionAnswering
...
def create_filter_conditions(args):
    ...
    unfreeze_intermediate_idxs = parse_range(args.unfreeze_intermediate_range)
    unfreeze_embedding_components = [f"{type}_embeddings" for type in parse_list(args.unfreeze_embedding_components)]
    unfreeze_attn_components = list(itertools.product(*[parse_range(args.unfreeze_attn_range),
                                                        parse_list(args.unfreeze_attn_components)]))
    unfreeze_attn_dense_range = parse_range(args.unfreeze_attn_dense_range)
    unfreeze_output_dense_range = parse_range(args.unfreeze_output_dense_range)
    ...
    fns.append(lambda x: any(f".{idx}.intermediate." in x for idx in unfreeze_intermediate_idxs))
    fns.append(lambda x: any(f".{idx}.attention.self.{part}" in x for idx, part in unfreeze_attn_components))
    fns.append(lambda x: any(type in x for type in unfreeze_embedding_components))
    fns.append(lambda x: any(f".{idx}.attention.output.dense" in x for idx in unfreeze_attn_dense_range))
    fns.append(lambda x: any(f".{idx}.output.dense" in x for idx in unfreeze_output_dense_range))
...
def parse_list(string: str):
    ...
    assert ']' == string[-1]
    lst = []
    for elm in string[1:-1].split(","):
        if elm.strip() != "":
            lst.append(elm.strip().lower())
    return lst
...
if __name__ == "__main__":
    ...
    parser.add_argument("--unfreeze_layernorm_range", default="None", type=str,
                        help="unfreeze some layers' layernorm")
    # feedforward
    parser.add_argument("--unfreeze_intermediate_range", default="None", type=str,
                        help="unfreeze some layers' feedforward")
    parser.add_argument("--unfreeze_attn_dense_range", default="None", type=str,
                        help="unfreeze some self attn's feedforward")
    parser.add_argument("--unfreeze_output_dense_range", default="None", type=str,
                        help="unfreeze some self attn's feedforward")
    # embedding
    parser.add_argument("--unfreeze_embedding_components", default="None", type=str,
                        help="embedding type to unfreeze")
    # attention
    parser.add_argument("--unfreeze_attn_components", default="None", type=str,
                        help="self-attention component to unfreeze")
    parser.add_argument("--unfreeze_attn_range", default="None", type=str,
                        help="attention range to unfreeze ")
    # adapter parameter
